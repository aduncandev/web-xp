...
import { resolveLocalPage } from './localPages';
...
const DEFAULT_HOME_PAGE = 'http://www.msn.com/';
...
function InternetExplorer({ onClose, onSetHeader, filePath, initialUrl }) {
  const vfs = useVFS();
  ...
  useEffect(() => {
    if (showCustomErrorPage && onSetHeader) {
      onSetHeader({
        title: 'Navigation Canceled - Microsoft Internet Explorer',
      });
    }
  }, [showCustomErrorPage, onSetHeader]);
  ...
  const navigateToUrl = useCallback(
    (url, addToHistory = true) => {
      ...
      // Classic start-page hostnames resolve to the local period pages
      const localPage = resolveLocalPage(finalUrl);
      if (localPage) {
        finalUrl = localPage.canonicalUrl;
      }

      // IE6 shows the URL being opened in the title bar until the page loads
      if (onSetHeader) {
        onSetHeader({ title: `${finalUrl} - Microsoft Internet Explorer` });
      }

      // Re-navigating to the current URL must still reload the iframe
    ...
    [historyStack, historyIndex, currentUrl, onSetHeader],
  );

  // Local period pages navigate through the outer chrome via postMessage
  useEffect(() => {
    const onMessage = e => {
      if (!e.data || e.data.type !== 'ie-navigate') return;
  ...
  // Open an .html or .url file from the virtual file system
  ...
  useEffect(() => {
    ...
    if (filePath.toLowerCase().endsWith('.url')) {
      // .url files are INI-style: URL=https://…
      const match = content.match(/^URL=(.+)$/im);
      if (match) navigateToUrl(match[1].trim());
    } else {
  ...
  // Direct URL launches (Run box, cmd `start https://…`)
  const loadedInitialUrl = useRef(null);
  useEffect(() => {
    if (!initialUrl || loadedInitialUrl.current === initialUrl) return;
    loadedInitialUrl.current = initialUrl;
  ...
  // '<page title or host> - Microsoft Internet Explorer', like real IE6
  const updateWindowTitle = () => {
    ...
    if (!pageTitle) {
      const localPage = resolveLocalPage(currentUrl);
      if (localPage) pageTitle = localPage.title;
    }
    if (!pageTitle) {
      try {
        pageTitle = new URL(currentUrl).host;
      } catch (e) {
        pageTitle = '';
      }
    }
    if (!pageTitle && currentUrl.startsWith('blob:') && filePath) {
      pageTitle = filePath;
    }
    if (!pageTitle) pageTitle = currentUrl;
    onSetHeader({ title: `${pageTitle} - Microsoft Internet Explorer` });
  };
  ...
  const currentLocalPage = resolveLocalPage(currentUrl);
  ...
  return (
    <Div>
      ...
      <section className="ie__address_bar">
        ...
        <XPTooltip text={`Go to "${addressBarInput}"`}>
          <div
      ...
      <div className="ie__content">
        {showCustomErrorPage ? (
          ...
          <iframe
            ...
            ref={iframeRef}
            src={currentLocalPage ? undefined : currentUrl}
            srcDoc={currentLocalPage ? currentLocalPage.html : undefined}
