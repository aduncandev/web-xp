...
function MineSweeperView({
  ...
  // Removed duplicate useEffect for mouseup listener
  return (
...
export default styled(MineSweeperView)`
  img {
    pointer-events: none; /* Prevents images from interfering with mouse events on parent */
  }
  .mine__options {
    height: 20px;
  ...
  /* --- MODIFIED --- */
  .mine__digits__outer {
    width: 40px; /* Accommodates three 13px-wide digits + 1px right padding/border */
    height: 24px; /* Accommodates 23px-high digits + 1px bottom padding/border */
    /* The following border creates the "sunken" effect for the digit display itself */
    border-top: rgb(128, 128, 128) solid 1px;
    border-left: rgb(128, 128, 128) solid 1px;
    ...
    padding: 0; /* Remove padding if border is handling the sunken look */
  ...
  .mine__face__outer {
    ...
    border-radius: 2px;
    transform: translateX(1px); /* Slight adjustment for centering */
  }
  .mine__face {
    border-radius: 2px;
    height: 100%;
  ...
  .mine__ceil {
    position: relative; /* For absolute positioning of images within */
    width: 16px;
    height: 16px;
