...
import { createGaplessLoop } from '../../audioLoop';
...
function Egg() {
  const gifSrc = `${import.meta.env.BASE_URL}gifs/tree.gif`;
  const musicSrc = `${import.meta.env.BASE_URL}music/man.ogg`;
  const soundSrc = `${import.meta.env.BASE_URL}music/egg.mp3`;

  const loopRef = useRef(null);
  const containerRef = useRef(null);
  const { effectiveVolume, applyVolume } = useVolume();
  const effectiveVolumeRef = useRef(effectiveVolume);
  effectiveVolumeRef.current = effectiveVolume;
  const vfs = useVFS();
  // Owner captured at mount so a window surviving a fast-user-switch in a
  // hidden session keeps reading/writing ITS user's hive, never the active
  // user's.
  ...
  // The room's music loops gaplessly (Web Audio). If the autoplay policy
  // suspends the context, the first click anywhere in the room resumes it.
  useEffect(() => {
    const loop = createGaplessLoop(musicSrc);
    loopRef.current = loop;
    loop.start(effectiveVolumeRef.current);
    const el = containerRef.current;
    const resume = () => loop.resume();
    if (el) el.addEventListener('click', resume);
    return () => {
      if (el) el.removeEventListener('click', resume);
      loop.stop();
      loopRef.current = null;
    };
    // Mount-only: volume changes are handled by the effect below
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (loopRef.current) loopRef.current.setVolume(effectiveVolume);
  }, [effectiveVolume]);
  ...
  const handleTreeClick = () => {
    if (dialogueStep === 0) {
      ...
      if (loopRef.current) loopRef.current.resume();
    }
  };
